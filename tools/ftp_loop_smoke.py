...
class FtpClient:
    ...
    def read_expect(self, prefix, context):
        ...
        if not lines[-1].startswith(prefix):
            raise RuntimeError(f"{context} expected {prefix}, got: {' | '.join(lines)}")
...
def capture_netinfo(monitor, log, offset, timeout_s, label):
    ...
    return wait_for_log(log, offset, "tcp buffers:", timeout_s)


def loop_payload(iteration, size):
    base = f"ftp loop iteration {iteration}\r\n".encode("ascii")
    if len(base) >= size:
        return base[:size]
...
def run_ftp_iteration(args, iteration):
    ftp = FtpClient(args.host, args.port, args.timeout)
    stor_path = f"{args.stor_prefix}{iteration:04d}.TXT"
    payload = loop_payload(iteration, args.payload_size)
    try:
        print(f"FTP loop iteration {iteration + 1}/{args.iterations}")
        ftp.login()
        ...
        ftp.upload(f"STOR {stor_path}", payload)
        print(f"STOR uploaded bytes: {len(payload)}")

        uploaded = ftp.download(f"RETR {stor_path}")
        if uploaded != payload:
            raise RuntimeError(
                f"uploaded file mismatch for {stor_path}: "
                f"expected {len(payload)} bytes, got {len(uploaded)}"
            )

        ftp.command(f"DELE {stor_path}", "250")
        ftp.command("QUIT", "221")
    finally:
        ftp.close()


def run_ftp_loop_smoke(args):
    ...
    try:
        with open(args.serial, "r", encoding="utf-8", errors="replace") as log:
            ...
            for i in range(args.iterations):
                run_ftp_iteration(args, i)
                offset = capture_netinfo(
                    monitor,
                    log,
                    offset,
                    args.timeout,
                    f"after iteration {i + 1}",
                )

        print("FTP loop smoke PASS")
        ok = True
        return 0
    except Exception as exc:
        print(f"FTP loop smoke FAIL: {exc}", file=sys.stderr)
        return 1
    finally:
...
def main():
    parser = argparse.ArgumentParser(description="SmallOS repeated FTP passive smoke")
