...
def main() -> int:
    ...
    expect(partition_table_offset + 2 * partition_entry_size <= boot_sector_size,
           "partition table must fit inside the boot sector")
    ...
    print(f"  partition entry sz  = {partition_entry_size}")
    return 0
