...
def main() -> int:
    ...
    loader2_sectors_patch_offset = parse_equ(boot_asm, "LOADER2_SECTORS_PATCH_OFFSET")
    fat16_lba_patch_offset = parse_equ(boot_asm, "FAT16_LBA_PATCH_OFFSET")
    ...
    expect(loader2_sectors_patch_offset + 4 <= boot_sector_size,
           "loader2 sector-count patch field must fit inside the boot sector")
    expect(fat16_lba_patch_offset + 4 <= boot_sector_size,
           "FAT16 patch field must fit inside the boot sector")
    ...
    expect(f"[org 0x{loader2_load_addr:x}]" in loader2_asm,
           f"loader2.asm must be assembled with org 0x{loader2_load_addr:x}")
    ...
    print(f"  loader2 patch off   = {loader2_sectors_patch_offset}")
    print(f"  fat16 patch off     = {fat16_lba_patch_offset}")
    return 0
