...
static int sh_parse_ip(const char* text, unsigned int* out_ip) {
...
static void sh_put_hex_digit(unsigned int v) {
    u_putc((char)(v < 10u ? '0' + v : 'A' + (v - 10u)));
}

static void sh_put_hex32(unsigned int value) {
    int started = 0;
    u_puts("0x");
    for (int shift = 28; shift >= 0; shift -= 4) {
        ...
        if (digit || started || shift == 0) {
            started = 1;
            sh_put_hex_digit(digit);
        }
    }
}

static void sh_put_u64_hex(unsigned long long value) {
...
static void sh_put_ip(unsigned int ip) {
...
static void sh_put_mac(const unsigned char mac[6]) {
    for (unsigned int i = 0; i < 6u; i++) {
        sh_put_hex_digit((mac[i] >> 4) & 0xFu);
        sh_put_hex_digit(mac[i] & 0xFu);
        if (i != 5u) u_putc(':');
    }
}
...
static void sh_builtin_meminfo(void) {
    sys_meminfo_t info;
    ...
    u_puts("heap:   base ");
    sh_put_hex32(info.heap_base);
    u_puts("  top ");
    sh_put_hex32(info.heap_top);
    u_puts("  used ");
...
static void sh_builtin_memmap(void) {
    sys_e820_entry_t entry;
    int count = sys_e820_entry(0, &entry);
    ...
    for (int i = 0; i < count; i++) {
        if (sys_e820_entry((uint32_t)i, &entry) < 0) break;
        u_put_uint((uint32_t)i);
        u_puts(": base ");
        sh_put_u64_hex(entry.base);
        u_puts("  length ");
        sh_put_u64_hex(entry.length);
        u_puts("  type ");
        u_put_uint(entry.type);
        u_putc(' ');
        if (entry.type == 1u) u_puts("usable");
        else if (entry.type == 2u) u_puts("reserved");
        else if (entry.type == 3u) u_puts("acpi");
        else if (entry.type == 4u) u_puts("nvs");
        else if (entry.type == 5u) u_puts("bad");
        else u_puts("other");
        u_putc('\n');
    }
}

static void sh_builtin_netinfo(void) {
    sys_netinfo_t info;
    ...
    if (info.ipv4_configured) {
        ...
        sh_put_ip(info.ip);
        u_puts(" mask=");
        sh_put_ip(info.netmask);
        u_puts(" gw=");
        sh_put_ip(info.gateway);
        if (info.dns) {
            u_puts(" dns=");
            sh_put_ip(info.dns);
        }
...
static void sh_builtin_ataread(int argc, char* argv[]) {
    unsigned int lba = 0;
    unsigned char sector[512];

    if (argc >= 2 && !sh_parse_uint(argv[1], &lba)) {
    ...
    for (unsigned int i = 0; i < 32u; i++) {
        sh_put_hex_digit((sector[i] >> 4) & 0xFu);
        sh_put_hex_digit(sector[i] & 0xFu);
        u_putc(' ');
        if (i == 15u) u_puts("\n  ");
    }
    u_putc('\n');
    ...
    if (lba == 0u) {
        ...
        u_puts("sig: 0x");
        sh_put_hex_digit((sector[510] >> 4) & 0xFu);
        sh_put_hex_digit(sector[510] & 0xFu);
        u_putc(' ');
        u_puts("0x");
        sh_put_hex_digit((sector[511] >> 4) & 0xFu);
        sh_put_hex_digit(sector[511] & 0xFu);
...
static int sh_builtin(int argc, char* argv[], int* should_exit) {
    ...
    if (sh_streq(argv[0], "meminfo")) {
        sh_builtin_meminfo();
        return 1;
    }
    if (sh_streq(argv[0], "memmap")) {
        sh_builtin_memmap();
        return 1;
    }
    if (sh_streq(argv[0], "netinfo")) {
        sh_builtin_netinfo();
        return 1;
    }
    if (sh_streq(argv[0], "dhcp")) {
        sys_net_op_request_t req;
        memset(&req, 0, sizeof(req));
        req.op = SYS_NET_OP_DHCP;
        u_puts(sys_net_op(&req) > 0 ? "dhcp: ok\n" : "dhcp: failed\n");
        return 1;
    }
    if (sh_streq(argv[0], "netsend")) {
        sys_net_op_request_t req;
        memset(&req, 0, sizeof(req));
        req.op = SYS_NET_OP_SEND_TEST_FRAME;
        if (sys_net_op(&req) > 0) u_puts("netsend: queued test frame\n");
        else u_puts("netsend: failed\n");
        return 1;
    }
    if (sh_streq(argv[0], "netrecv")) {
        sys_net_op_request_t req;
        memset(&req, 0, sizeof(req));
        req.op = SYS_NET_OP_POLL_ONCE;
        if (sys_net_op(&req) > 0) u_puts("netrecv: dispatched packet\n");
        else u_puts("netrecv: no packet\n");
        return 1;
    }
    if (sh_streq(argv[0], "arpgw")) {
        sys_net_op_request_t req;
        int rc;
        memset(&req, 0, sizeof(req));
        ...
        sh_put_ip(req.next_hop_ip);
        u_puts(" from ");
        sh_put_ip(req.sender_ip);
        u_putc('\n');
        if (rc == 0) {
    ...
    if (sh_streq(argv[0], "ping")) {
        sys_net_op_request_t req;
        int rc;
        memset(&req, 0, sizeof(req));
        req.op = SYS_NET_OP_PING;
        if (argc >= 2 && !sh_parse_ip(argv[1], &req.target_ip)) {
    ...
    if (sh_streq(argv[0], "netcheck")) {
        sys_net_op_request_t req;
        int rc;
        memset(&req, 0, sizeof(req));
        req.op = SYS_NET_OP_ARP;
        u_puts("netcheck: gateway arp\n");
        rc = sys_net_op(&req);
        if (rc <= 0) {
            u_puts(rc < 0 ? "netcheck: IPv4 gateway is not configured\n"
                          : "netcheck: gateway arp failed\n");
            return 1;
        }
        u_puts("netcheck: gateway arp ok\n");

        memset(&req, 0, sizeof(req));
        req.op = SYS_NET_OP_PING;
        u_puts("netcheck: gateway ping\n");
        rc = sys_net_op(&req);
        if (rc <= 0) {
            u_puts(rc < 0 ? "netcheck: IPv4 gateway is not configured\n"
                          : "netcheck: gateway ping failed\n");
            return 1;
        }
        u_puts("netcheck: gateway ping ok\n");
    ...
    if (sh_streq(argv[0], "ataread")) {
        sh_builtin_ataread(argc, argv);
        return 1;
    }
