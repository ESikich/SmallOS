...
static files_state_t* state_of(gui_window_t* window) {
    return (files_state_t*)window->state;
}

static void load_directory(gui_window_t* window) {
    files_state_t* state = state_of(window);
...
static void confirm_dialog(gui_window_t* window) {
    files_state_t* state = state_of(window);
    ...
    if (rc == 0) load_directory(window);
}

static void properties(gui_window_t* window) {
    files_state_t* state = state_of(window); char path[256]; sys_stat_info_t info;
...
static void files_open(gui_app_context_t* context, const char* argument) {
    files_state_t* state = state_of(context->window); (void)argument;
    copy_text(state->cwd, "/", sizeof(state->cwd));
    copy_text(state->status, "Double-click files to open", sizeof(state->status));
    state->selected = -1;
    state->hovered = -1;
    load_directory(context->window);
}

static void files_draw(gfx_surface_t* surface, gui_app_context_t* context,
                       int mouse_x, int mouse_y) {
    gui_window_t* window = context->window; files_state_t* state = state_of(window);
    int bx = window->x, by = window->y + TITLE_H, bw = window->w;
    int bh = window->h - TITLE_H, row_top = by + 36, status_h = 13;
    ...
    gui_widget_button(surface, gui_rect_make(bx + 4, by + 15, 42, 18), "New", (gui_widget_state_t){0}, theme, gui_app_services_draw_text);
    gui_widget_button(surface, gui_rect_make(bx + 50, by + 15, 54, 18), "Rename", (gui_widget_state_t){0,0,0,state->selected < 0 || same_text(state->rows[state->selected], "..")}, theme, gui_app_services_draw_text);
    gui_widget_button(surface, gui_rect_make(bx + 108, by + 15, 48, 18), "Delete", (gui_widget_state_t){0,0,0,state->selected < 0 || same_text(state->rows[state->selected], "..")}, theme, gui_app_services_draw_text);
    gui_widget_button(surface, gui_rect_make(bx + 160, by + 15, 62, 18), "Properties", (gui_widget_state_t){0,0,0,state->selected < 0}, theme, gui_app_services_draw_text);
    ...
    gui_widget_scrollbar(surface, gui_rect_make(bx + bw - 10, by + 36, 10, row_area), state->row_count, visible, state->scroll, (gui_widget_state_t){0,window->pressed_widget == 1,0,0}, theme);
    ...
    if (state->dialog != FILES_DIALOG_NONE) {
        gui_rect_t modal = gui_rect_make(bx + (bw - 250) / 2, by + (bh - 84) / 2, 250, 84);
        const char* title = state->dialog == FILES_DIALOG_NEW ? "New Folder" : state->dialog == FILES_DIALOG_RENAME ? "Rename" : "Delete";
        gui_widget_modal(surface, modal, title, state->dialog == FILES_DIALOG_DELETE ? "Delete selected item?" : "Name:", theme, gui_app_services_draw_text);
        if (state->dialog != FILES_DIALOG_DELETE) gui_widget_text_field(surface, gui_rect_make(modal.x + 8, modal.y + 39, modal.w - 16, 18), state->dialog_input.text, state->dialog_input.cursor, (gui_widget_state_t){0,0,1,0}, theme, gui_app_services_draw_text);
        gui_app_services_draw_text(surface, modal.x + 8, modal.y + 67, "Enter: confirm   Esc: cancel", COL_SUBTEXT);
    }
    gui_canvas_rect(surface, window->x, window->y, window->w, window->h, COL_FRAME);
}

static unsigned int files_event(gui_app_context_t* context,
                                const gui_app_event_t* event) {
    gui_window_t* window = context->window; files_state_t* state = state_of(window);
    int row_area = window->h - TITLE_H - 49, visible = row_area / ROW_H;
    if (visible < 1) visible = 1;
    int max_scroll = state->row_count - visible; if (max_scroll < 0) max_scroll = 0;
    gui_rect_t track = gui_rect_make(window->w - 10, 36, 10, row_area);
    gui_rect_t thumb = gui_widget_scroll_thumb(track, state->row_count, visible, state->scroll);
    if (event->type == GUI_APP_EVENT_POINTER_MOVE && window->pressed_widget != 1) {
        int hovered = -1;
        if (event->x >= 0 && event->x < window->w - 12 && event->y >= 36 &&
    ...
    if (event->type == GUI_APP_EVENT_WHEEL) state->scroll -= event->wheel * 3;
        ...
        if (state->dialog != FILES_DIALOG_NONE) {
            if (event->key == KEY_ESC) state->dialog = FILES_DIALOG_NONE;
            else if (event->key == KEY_ENTER) confirm_dialog(window);
            else if (state->dialog != FILES_DIALOG_DELETE) (void)edit_dialog(&state->dialog_input, event);
            return GUI_APP_RESULT_HANDLED | GUI_APP_RESULT_REDRAW;
        }
        if ((event->modifiers & SYS_INPUT_KEY_CTRL) && (event->ascii == 'n' || event->ascii == 'N')) { gui_text_input_init(&state->dialog_input, "New Folder"); state->dialog = FILES_DIALOG_NEW; }
        else if (event->key == KEY_F2 && state->selected >= 0 && !same_text(state->rows[state->selected], "..")) { gui_text_input_init(&state->dialog_input, state->rows[state->selected]); state->dialog = FILES_DIALOG_RENAME; }
        else if (event->key == KEY_DELETE && state->selected >= 0 && !same_text(state->rows[state->selected], "..")) state->dialog = FILES_DIALOG_DELETE;
        else if ((event->modifiers & SYS_INPUT_KEY_ALT) && event->key == KEY_ENTER) properties(window);
    ...
    } else if (event->type == GUI_APP_EVENT_POINTER_MOVE && window->pressed_widget == 1) {
        state->scroll = gui_widget_scroll_offset(track, state->row_count, visible, event->y, state->scroll_drag_offset);
    } else if (event->type == GUI_APP_EVENT_POINTER_UP && window->pressed_widget == 1) window->pressed_widget = 0;
    else if (event->type == GUI_APP_EVENT_POINTER_DOWN) {
        if (gui_widget_hit(gui_rect_make(4,15,42,18), event->x,event->y)) { gui_text_input_init(&state->dialog_input,"New Folder"); state->dialog=FILES_DIALOG_NEW; }
        else if (state->selected >= 0 && !same_text(state->rows[state->selected],"..") && gui_widget_hit(gui_rect_make(50,15,54,18),event->x,event->y)) { gui_text_input_init(&state->dialog_input,state->rows[state->selected]); state->dialog=FILES_DIALOG_RENAME; }
        else if (state->selected >= 0 && !same_text(state->rows[state->selected],"..") && gui_widget_hit(gui_rect_make(108,15,48,18),event->x,event->y)) state->dialog=FILES_DIALOG_DELETE;
        else if (state->selected >= 0 && gui_widget_hit(gui_rect_make(160,15,62,18),event->x,event->y)) properties(window);
        else if (gui_widget_hit(track,event->x,event->y)) {
            if (gui_widget_hit(thumb,event->x,event->y)) { window->pressed_widget=1; state->scroll_drag_offset=event->y-thumb.y; }
