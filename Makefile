...
USER_PROGS=echo about uptime halt reboot hello ticks args runelf_test readline exec_test fileread compiler_demo heapprobe statprobe fileprobe cwdprobe stdioprobe dirprobe errnoprobe fault sleep_test ptrguard spinwkr preempt_test tcpecho ftpd
...
.PHONY: all dirs run run-gtk run-sdl run-tap run-headless run-headless-tap test ftp-smoke smoke smoke-reboot smoke-halt clean boot-layout-check image-layout-check verify reset-disk tinycc-host tinycc-host-clean
