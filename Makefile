...
USER_DYNAMIC_WITH_CRT0=crtprobe mathprobe pipeprobe dupprobe forkprobe cowprobe memleakprobe procscaleprobe lifetimeprobe execveprobe envprobe compatprobe permprobe atprobe ttyprobe rsrcprobe sessprobe mountprobe netbbprobe
...
USER_DYNAMIC_PIE_WITH_CRT0=crtprobe mathprobe pipeprobe dupprobe forkprobe cowprobe memleakprobe procscaleprobe lifetimeprobe execveprobe envprobe compatprobe permprobe atprobe ttyprobe rsrcprobe sessprobe mountprobe netbbprobe
...
$(OBJ_DIR)/kernel/syscall.o: KERNEL_CFLAGS += -Os
...
$(BIN_DIR)/cowprobe.elf: $(OBJ_DIR)/user/cowprobe.o $(USER_CRT0_OBJ) $(USER_LIB_ARCHIVES) | dirs
	$(LD) $(USER_LDFLAGS) $(filter %.o,$^) $(USER_LINK_LIBS) -o $@
...
$(BIN_DIR)/compatprobe.elf: $(OBJ_DIR)/user/compatprobe.o $(USER_CRT0_OBJ) $(USER_LIB_ARCHIVES) | dirs
	$(LD) $(USER_LDFLAGS) $(filter %.o,$^) $(USER_LINK_LIBS) -o $@
...
BUSYBOX_NET_SMOKE_HOST_WHOIS_PORT?=18083
BUSYBOX_NET_SMOKE_HOST_FTP_PORT?=18084
BUSYBOX_NET_SMOKE_HOST_FTP_DATA_PORT?=18085
BUSYBOX_NET_SMOKE_HOST_TFTP_PORT?=18086
BUSYBOX_NET_SMOKE_GUEST_TCPSVD_PORT?=18087
BUSYBOX_NET_SMOKE_GUEST_TFTPD_PORT?=18088
PYTHON3=python3
...
busybox-net-smoke:
	...
	@if [ -f $(PIDFILE) ]; then kill "$$(cat $(PIDFILE))" 2>/dev/null || true; fi
	rm -f $(SERIAL_LOG) $(MONITOR_SOCK) $(PIDFILE)
	$(MAKE) run-headless SERIAL_CONSOLE=1 QEMU_NET_HOSTFWD=',hostfwd=tcp::$(BUSYBOX_NET_SMOKE_GUEST_HTTP_PORT)-:$(BUSYBOX_NET_SMOKE_GUEST_HTTP_PORT),hostfwd=tcp::$(BUSYBOX_NET_SMOKE_GUEST_TCPSVD_PORT)-:$(BUSYBOX_NET_SMOKE_GUEST_TCPSVD_PORT),hostfwd=udp::$(BUSYBOX_NET_SMOKE_GUEST_TFTPD_PORT)-:$(BUSYBOX_NET_SMOKE_GUEST_TFTPD_PORT)'
	$(PYTHON3) tools/busybox_net_smoke.py \
		...
		--host-whois-port $(BUSYBOX_NET_SMOKE_HOST_WHOIS_PORT) \
		--host-ftp-port $(BUSYBOX_NET_SMOKE_HOST_FTP_PORT) \
		--host-ftp-data-port $(BUSYBOX_NET_SMOKE_HOST_FTP_DATA_PORT) \
		--host-tftp-port $(BUSYBOX_NET_SMOKE_HOST_TFTP_PORT) \
		--guest-tcpsvd-port $(BUSYBOX_NET_SMOKE_GUEST_TCPSVD_PORT) \
