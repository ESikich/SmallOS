...
busybox-net-smoke:
	...
	@if [ -f $(PIDFILE) ]; then kill "$$(cat $(PIDFILE))" 2>/dev/null || true; fi
	rm -f $(SERIAL_LOG) $(MONITOR_SOCK) $(PIDFILE)
	$(MAKE) run-headless SERIAL_CONSOLE=1 QEMU_NET_HOSTFWD=',hostfwd=tcp::$(BUSYBOX_NET_SMOKE_GUEST_HTTP_PORT)-:$(BUSYBOX_NET_SMOKE_GUEST_HTTP_PORT),hostfwd=tcp::$(BUSYBOX_NET_SMOKE_GUEST_TCPSVD_PORT)-:$(BUSYBOX_NET_SMOKE_GUEST_TCPSVD_PORT)'
