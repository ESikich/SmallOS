...
#define HELP_FILE_LONG_SIZE 4

static void write_help_int(FILE *file, int value)
   {
...
static void write_help_long(FILE *file, unsigned long value)
   {
   unsigned char buf[HELP_FILE_LONG_SIZE];
...
#ifdef XFRACT
#define putw( x1, x2 )  write_help_int(x2, x1)
#endif
...
void calc_offsets(void)    /* calc file offset to each topic */
   ...
   offset = sizeof(int) +           /* max_pages */
            ...
            num_topic*HELP_FILE_LONG_SIZE +/* offsets to each topic */
...
void _write_help(FILE *file)
   ...
   write_help_long(file, HELP_SIG);
   write_help_int(file, version);
   ...
   for (t=0; t<num_topic; t++)
      write_help_long(file, topic[t].offset);
