from .common import case


CASES = [
    case(
        name="mkdir_rmdir",
        must_contain=[
            "shelltest: mkdir begin",
            "mkdir: TESTDIR",
            "shelltest: mkdir end",
            "shelltest: ls_newdir begin",
            "ext2 directory: TESTDIR",
            "shelltest: ls_newdir end",
            "shelltest: rmdir begin",
            "rmdir: TESTDIR",
            "shelltest: rmdir end",
            "shelltest: ls_removed begin",
            "ext2: not found: TESTDIR",
            "shelltest: ls_removed end",
        ],
        timeout=60.0,
    ),
    case(
        name="mkdir_rmdir_nested",
        must_contain=[
            "shelltest: mkdir_nested_parent begin",
            "mkdir: NESTPARENT",
            "shelltest: mkdir_nested_parent end",
            "shelltest: mkdir_nested_child begin",
            "mkdir: NESTPARENT/CHILD",
            "shelltest: mkdir_nested_child end",
            "shelltest: ls_nested begin",
            "ext2 directory: NESTPARENT",
            "CHILD/",
            "shelltest: ls_nested end",
            "shelltest: rmdir_nested_child begin",
            "rmdir: NESTPARENT/CHILD",
            "shelltest: rmdir_nested_child end",
            "shelltest: rmdir_nested_parent begin",
            "rmdir: NESTPARENT",
            "shelltest: rmdir_nested_parent end",
            "shelltest: ls_nested_removed begin",
            "ext2: not found: NESTPARENT",
            "shelltest: ls_nested_removed end",
        ],
        timeout=60.0,
    ),
]
