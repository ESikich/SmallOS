...
Loader2 is currently at `0x20000`. The kernel loads to `0x1000`. The stage-2 stack top is generated by the build (`SP=0xFF00`, physical top `0x2FF00`). The kernel must not grow large enough to overwrite loader2 or the stack during the INT 0x13 read:

```text
safe kernel size = (0x20000 - 0x1000) / 512 sectors
                 = 248 sectors = 124 KiB
```

The required invariant is that `0x1000 + kernel_sectors * BOOT_SECTOR_SIZE` must stay below both the loader2 load address and the generated stage-2 stack top. If the kernel exceeds 248 sectors, the build fails before image assembly. To raise the ceiling, move stage 2 higher and update the matching constants in `Makefile` and `loader2.asm`.
...
* Boot/layout work — the fixed disk layout is still intentionally explicit; the loader now sits at `0x20000` and the current kernel ceiling is 248 sectors
