...
Loader2 is currently at `0xB000`. The kernel loads to `0x1000`. The stage-2 stack top is generated by the build. The kernel must not grow large enough to overwrite loader2 during the INT 0x13 read:

```text
safe kernel size = (0xF000 - 0x1000) / 512 sectors
                 = 112 sectors = 56 KB
```

The required invariant is `0x1000 + kernel_sectors * BOOT_SECTOR_SIZE < loader2 load address`. If the kernel exceeds 112 sectors, the build fails before image assembly. To raise the ceiling, move the generated stage-2 stack top higher and update the matching constants in `Makefile` and `loader2.asm`.
