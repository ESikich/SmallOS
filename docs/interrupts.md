...
- `process_key_consumer` (registered by `process_set_foreground`) - pushes ASCII into `kb_buf` for `SYS_READ`; Ctrl+C signals or terminates the foreground process group unless that foreground console owner is waiting in `SYS_READ_RAW`, in which case byte `0x03` is queued for the raw reader. Ctrl+Z either detaches a native shell-owned foreground wait or sends `SIGTSTP` to the foreground process group so POSIX-style shells can observe stopped jobs.
...
One early exception is the storage probe window in `boot_mount_ext2()`: it
...
The first `sched_start()` stack switch also runs with IF masked; kernel-task
bootstrap enables interrupts once execution is on the task stack. USB boot
keyboard/mouse discovery happens later from the scheduler-owned `usb` task,
which polls OHCI interrupt transfers, injects keyboard reports as translated
set-1 scancodes, and injects mouse reports into the normal mouse state.
