...
The required invariant is that `0x1000 + kernel_sectors * 512` must stay below both the loader2 load address and the generated stage-2 stack top. If the kernel exceeds 120 sectors, the build fails before image assembly. To raise the ceiling, move stage 2 higher and update the matching constants in `Makefile` and `loader2.asm`.
