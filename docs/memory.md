...
The required invariant is `0x1000 + kernel_sectors * 512 < loader2 load address`. If the kernel exceeds 112 sectors, the build fails before image assembly. To raise the ceiling, move the generated stage-2 stack top higher and update the matching constants in `Makefile` and `loader2.asm`.
