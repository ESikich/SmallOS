...
* Load stage 2 from disk to `0xB000`
...
Loader2 sits at `0xB000`. The kernel loads to `0x1000`. The stage-2 stack top is generated by the build, and the kernel must not grow large enough to overwrite loader2 during the INT 0x13 read:

```text
safe kernel size = (0xF000 - 0x1000) / 512 = 112 sectors = 56 KB
```

If the kernel exceeds 112 sectors, the build fails before image assembly. To raise the ceiling, move the generated stage-2 stack top higher and update the matching constants in `Makefile` and `loader2.asm`.
...
Segment registers loaded, stack set to `0xF0000`, then execution jumps to the kernel entry point.
...
```text
0x00007C00   stage 1 (boot.asm) — done after jump to 0xB000
0x0000B000   stage 2 (loader2.asm) — done after jump to 0x1000
0x00001000   kernel entry point (_start in kernel_entry.asm)
0x000F0000   stack top (set up by loader2's init_pm)
```
...
* `0x1000 + kernel_sectors * 512 < loader2 load address` (currently `0xB000`)
...
The kernel load overwrote loader2 mid-transfer. The kernel has grown beyond the generated stage-2 ceiling. Move the stage-2 stack top higher and update the matching constants in `Makefile` and `loader2.asm`.
...
```text
Stage 1  →  load stage 2 (CHS, fixed-size loader, to 0xB000)
...
* Support kernels larger than 112 sectors (raise the generated stage-2 stack top and update the matching build constants)
