...
15. `process_create_kernel_task("bootsvc", ...)` — wait until ext2 is mounted and the splash has been presented, then queue the default FTP, cserve, and TinySSH user services. Service status is appended to `/var/log/boot.txt` without drawing over the splash.
...
handles are shared open-file descriptions initialized by `vfs_file_init()` and
implemented in `vfs.c`, pipe handles point at refcounted one-page ring buffers
with read/write wait queues, socket handles point at `socket_t` objects
implemented in `socket.c`, and socket blocking readiness is tracked by
socket-owned wait queues. `dup*()` and `fork()` copy descriptors while sharing
file offsets/status flags; `FD_CLOEXEC` is per descriptor and honored by
`execve()`. The TCP
...
page flip instead of copying the full frame through the kernel. The BGA
framebuffer backend waits for a fresh vertical-retrace edge before changing the
scanout offset, which keeps page flips out of active scanout and reduces top-edge
tearing for ports that present frequently.
...
SmallOS-hosted TinyCC and hosted GNU binutils linker flows.
...
The ext2 start LBA is computed during final image assembly by `mkimage` as `kernel_lba + kernel_sectors` and written into partition entry 1 of the MBR partition table. `loader2.asm` reads partition entry 0 to load the kernel. With the default `BOOT_RAMDISK_FALLBACK=never` policy, normal VM/IDE boots skip the fallback preload. `BOOT_RAMDISK_FALLBACK=auto` preloads the used portion of partition entry 1 only when EDD does not identify the boot drive as USB or ATA. USB image and explicit fallback targets force `BOOT_RAMDISK_FALLBACK=always` so hardware-oriented builds retain a BIOS preload path, while direct QEMU USB storage runs keep it disabled to test `usb0` directly. At runtime, the storage policy selects a block backend, reads sector 0 through that backend, extracts the ext2 partition metadata, and uses it to locate the live volume. It tries writable ATA first, then USB mass storage (`usb0`, read-only), then the preloaded RAM fallback when one exists.
...
# SYS_EXEC / SYS_FORK / SYS_EXECVE

A running user process can invoke a named child through the legacy spawn-style
ELF creation path:
...
This is async spawn, not blocking foreground execution. POSIX-shaped userland
...
| hardware summary   | cpuz command via CPUID and diagnostic syscalls          |
...
SYS_OPEN / SYS_OPEN_MODE / SYS_OPEN_CREATE_MODE / SYS_OPENAT_CREATE_MODE / SYS_CLOSE / SYS_FREAD — dynamic PMM-backed per-process descriptor table backed by readable/writable handle ops; fd 0/1/2 are console handles, user-opened files start at fd 3+, directory file descriptors can anchor relative `*at()` paths, and VFS-backed file reads cache ext2 data in shared PMM-backed file descriptions until the final close
...
TCP service task — drains NIC RX, dispatches ARP/IPv4/TCP frames, advertises receive windows from per-connection RX rings, services retransmit timers for control handshakes, buffered TX payloads, active-open SYNs, and FIN paths, reaps idle FIN_WAIT sockets without applying a generic established-connection idle timeout, handles duplicate peer-FIN ACKs and close-driven final writes, and wakes socket wait queues
...
guest GNU binutils path — hosted `addr2line`, `ar`, `as`, `ld`, `nm`, `objcopy`, `objdump`, `ranlib`, `readelf`, `size`, `strings`, and `strip` are staged under `usr/bin`, with `binutilsprobe` exercising a generated ELF assemble/archive/ranlib/link/inspect/copy/strip/execute round trip
guest BusyBox/TinySSH path — `usr/bin/busybox` is built from unmodified upstream source and provides fallback Unix applets, `/bin/sh` runs BusyBox `ash` with basic job control, the SmallOS libc/sysroot supplies BusyBox-facing POSIX/GNU helper APIs, the root image exposes visible `/proc` and `/dev` mountpoints, virtual `/proc`, `/proc/net`, and `/dev` provide the small Linux-like status/device surface those tools need, ext2 supplies the link/symlink/metadata/node operations used by filesystem applets, kernel termios/session/credential/PTY compatibility backs the basic `init`/`login`/`getty` applet surface and TinySSH forced-PTY sessions, and kernel stopped-job/process-group semantics plus network ioctls/rtnetlink/raw ICMP/ARP-neighbor/UDP DNS/TFTP/IPv4 TCP/native DHCP back `ash`, `ifconfig`, `route`, `arp`, `hostname`, `ipcalc`, `netstat`, `nslookup`, `pscan`, `whois`, `ip`, `ping`, `nc`, plain HTTP `wget`, `ftpget`, `ftpput`, `tftp`, `tcpsvd`, `udpsvd`, `tftpd`, `httpd`, `udhcpc`, and boot-started TinySSH root public-key login
```
