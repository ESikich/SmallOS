...
driver owns listener/connection state plus lazy 4 KiB PMM-backed receive rings
for accepted streams; send-side queueing and retransmit buffering are still
future work. Console handles own terminal writes and keyboard-buffer reads.
...
TCP service task — drains NIC RX, dispatches ARP/IPv4/TCP frames, advertises receive windows from per-connection RX rings, services retransmit/idle timers, and wakes socket wait queues
